...
def ensure_font() -> bool:
    """Download DejaVuSans.ttf if missing. Returns True if font available."""
...
def _pdf_write_table(pdf: FPDF, df: pd.DataFrame, font_ok: bool, max_rows: int = 5):
    ...
    col_width = max(20, (pdf.w - 20) / col_count - 2)
    row_height = pdf.font_size * 1.5

    pdf.set_fill_color(200, 200, 200)
    for col in df.columns:
        pdf.cell(col_width, row_height, sanitize_for_pdf(col, font_ok, width=20),
...
def create_pdf_report(
    ...
    if isinstance(pdf_bytes, (bytes, bytearray)):
        ...
        buf.write(pdf_bytes.encode("latin-1", "ignore"))
    else:
        buf.write(b"")
