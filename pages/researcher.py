...
def app():
    ...
    model_type = st.sidebar.selectbox(
        "Model Type", ["RandomForest", "CNN (image)", "LSTM (time-series)"]
    )
    ...
    epochs = (
        st.sidebar.slider("Epochs (CNN/LSTM)", 1, 50, 5)
        if model_type != "RandomForest"
        else None
    )
    batch_size = (
        st.sidebar.slider("Batch Size (CNN/LSTM)", 8, 128, 32)
        if model_type != "RandomForest"
        else None
    )
    ...
    if st.button("🚀 Train & Evaluate Model", use_container_width=True):
        ...
        if model_type == "RandomForest":
            ...
            clf = model_utils.train_crop_model(
                X_train,
                y_train,
                seed=seed,
                ...
                model = tf.keras.Sequential(
                    [
                        tf.keras.layers.Conv2D(
                            16, (3, 3), activation="relu", input_shape=X.shape[1:]
                        ),
        ...
        # ---------------- LSTM ----------------
            ...
            if not TF_AVAILABLE:
                st.error("❌ TensorFlow is not available in this deployment. LSTM cannot run.")
            else:
    ...
    if st.session_state.get("researcher_last_model"):
        ...
        st.write(
            f"Type: **{info['type']}**, Seed: **{info['seed']}**, Test split: **{info['test_size']}**"
        )
